...
namespace unrealenginedecryption
{
	...
	public:
		...
		__forceinline std::string username(__int64 playerstate)
		{
			...
			read_array(Data, (uint8_t*)(NameBuffer), (Length * 2));
		...
		//					ctx->read_array((uintptr_t)ftext_data, (uintptr_t)ftext_buf, ftext_length * sizeof(wchar_t))// UPDATED BY H;
		...
		__forceinline std::string weapon(uint64_t CurrentWeapon) {
			...
			read_array((ULONG64)read<PVOID>(DisplayName + 0x38), (uint8_t*)WeaponName, WeaponLength * sizeof(wchar_t));
