...
public:
	...
	const bool Init(const BOOL PhysicalMode) {
		...
		this->hDriver = CreateFileA((("\\\\.\\ndis")), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
		if (this->hDriver != INVALID_HANDLE_VALUE) {
			...
			if (this->SharedBuffer = VirtualAlloc(0, sizeof(REQUEST_DATA), MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)) {
				UNICODE_STRING RegPath = RTL_CONSTANT_STRING(L"\\Registry\\Machine\\SOFTWARE\\ndis");
...
private:
	PVOID SharedBuffer;
	HANDLE hDriver;
	ULONG64 MAGGICCODE = 0x04745de7d73b6c96;
...
std::string readwtf(uintptr_t Address, void* Buffer, SIZE_T Size)
{
	...
	return std::string(name);// UPDATED BY H
}
